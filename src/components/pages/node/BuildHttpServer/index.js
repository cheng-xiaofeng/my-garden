    ...
    return (
      <div>
        ...
        <div className='articleConent'>
          <p>原文地址: <a target="_blank"  rel="noopener noreferrer" href="https://nodejs.dev/learn/build-an-http-server">https://nodejs.dev/learn/build-an-http-server</a></p>
          ...
          <p>简单分析一下,它包含<a target="_blank"  rel="noopener noreferrer" href="https://nodejs.org/api/http.html">http module</a></p>
          <p>我们使用这个模块建立http服务器</p>
          <p>服务器被设定监听一个特定的端口:3000.当服务器准备好以后,监听回调函数就会被调用</p>
          <p>在每个request进来的时候我们传的回调函数都会执行.无论什么时候接收到一个新的request,
            <a target="_blank"  rel="noopener noreferrer" href="https://nodejs.org/api/http.html#http_event_request">request事件</a>
            就会被调用,提供两个对象:一个request(一个<a target="_blank"  rel="noopener noreferrer" href="https://nodejs.org/api/http.html#http_class_http_incomingmessage">http.IncomingMessage对象</a>)
            一个是response(一个<a target="_blank"  rel="noopener noreferrer" href="https://nodejs.org/api/http.html#http_class_http_serverresponse">http.ServerResponse对象</a>)
          </p>
